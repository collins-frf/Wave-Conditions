...
thtype=0;
if prod(size(th))>1 % non-scalar
  if size(th,1)==1|size(th,2)==1 % theta vector
    th=th(:);
    thtype=1;
  end
...
if length(g)>1
  error('Finite depth in combination with 3D normalization (=> length(g)=2) is not implemented yet.')
end
% Newton's Method
...
if count == count_limit, 
  ...
  outstr = sprintf([str,'%13.8f'],hn(ix));
  fprintf(outstr);
  ind=ix;
else
  ind=[];
